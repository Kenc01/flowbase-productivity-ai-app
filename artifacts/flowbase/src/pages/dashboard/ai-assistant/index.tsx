...
import {
  Bot, Send, Mic, MicOff, Square, Loader2, CheckCircle2,
  AlertCircle, ArrowRight, Sparkles, Calendar, StickyNote,
  Kanban, LayoutTemplate, Plus, CornerDownLeft,
} from "lucide-react";
...
const SUGGESTIONS = [
  { icon: Kanban, label: "Create a task for tomorrow", color: "#7467F0" },
  { icon: Calendar, label: "Add meeting reminder on calendar", color: "#06B6D4" },
  { icon: StickyNote, label: "Summarize my notes", color: "#F59E0B" },
  { icon: Kanban, label: "Create a Kanban board", color: "#10B981" },
...
function ActionCard({ action, onNavigate }: { action: Action; onNavigate: (href: string) => void }) {
  const Icon = TOOL_ICONS[action.tool] ?? Sparkles;
  return (
    <div style={{
      display: "flex", alignItems: "flex-start", gap: 10,
      background: action.success ? "#f0fdf4" : "#fff1f2",
      border: `1px solid ${action.success ? "#bbf7d0" : "#fecdd3"}`,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: action.success ? "#15803d" : "#b91c1c", marginBottom: 2 }}>
          {TOOL_LABELS[action.tool] ?? "Action"} {action.success ? "created" : "failed"}
        </div>
        <div style={{ fontSize: 12, color: action.success ? "#166534" : "#991b1b" }}>{action.summary}</div>
      </div>
      {action.success && action.link && (
        <button
...
export default function AIAssistantPage() {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      setVoiceError("Microphone access denied. Please allow microphone use.");
    }
  };
  ...
  return (
    <>
      <style>{`
        ...
        .fb-voice-btn.recording { animation: fb-pulse 1.5s ease-in-out infinite; }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#1a1f36" }}>AI Assistant</div>
        ...
        {/* Messages area */}
        <div style={{
          flex: 1, overflowY: "auto", padding: isEmpty ? 0 : "24px",
          ...
          {isEmpty ? (
            ...
            <div style={{
              ...
              <p style={{ fontSize: 14, color: "#6b7280", margin: 0, marginBottom: 32, textAlign: "center", maxWidth: 420 }}>
                Your smart workspace assistant. Ask me anything, or tell me to create tasks, notes, calendar events, and more.
              </p>
                  ...
                  <div style={{ maxWidth: "78%", minWidth: 0 }}>
                    <div style={{
                      background: msg.role === "user"
                        ? "linear-gradient(135deg, #7467F0, #6366f1)"
                        : "#fff",
        ...
        {voiceError && (
          <div style={{
            ...
            <AlertCircle size={14} />
            {voiceError}
            <button onClick={() => setVoiceError(null)} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "#b91c1c", fontSize: 16 }}>×</button>
          </div>
        )}
        ...
        <div style={{
          ...
          {(recording || transcribing) && (
            <div style={{
              ...
              {transcribing ? (
                <><Loader2 size={14} color="#0284c7" style={{ animation: "spin 1s linear infinite" }} />
                  <span style={{ fontSize: 13, color: "#0284c7" }}>Transcribing your voice…</span></>
              ) : (
                <><div style={{ width: 8, height: 8, borderRadius: "50%", background: "#ef4444", animation: "fb-pulse 1.5s ease-in-out infinite" }} />
                  <span style={{ fontSize: 13, color: "#dc2626", fontWeight: 600 }}>Recording {formatTime(recSeconds)}</span>
                  <span style={{ fontSize: 12, color: "#9ca3af", marginLeft: 4 }}>Click stop when done</span></>
              )}
            </div>
          )}
          ...
          <div style={{
            ...
            background: "#f8f9ff",
            border: "1.5px solid #e0e0f0", borderRadius: 16,
            padding: "10px 12px",
            transition: "border-color 0.15s",
          }}>
            {/* Voice button */}
            <button
              className={`fb-voice-btn${recording ? " recording" : ""}`}
            ...
            >
              {transcribing
                ? <Loader2 size={17} color="#0284c7" style={{ animation: "spin 1s linear infinite" }} />
                : recording
            ...
            <textarea
              ...
              placeholder={recording ? "Recording…" : "Ask me anything, or say 'create a task for…'"}
              rows={1}
