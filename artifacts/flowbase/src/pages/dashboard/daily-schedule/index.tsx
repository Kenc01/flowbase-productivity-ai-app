...
function blocksToScheduleText(blocks: ScheduleBlock[]) {
  const sorted = [...blocks].sort((a, b) => toMinutes(a.startHour, a.startMin) - toMinutes(b.startHour, b.startMin));
  if (sorted.length === 0) return "No schedule set yet.";
  return sorted.map(b =>
    `${fmtTime(b.startHour, b.startMin)} – ${fmtTime(b.endHour, b.endMin)}: ${b.label} (${TYPE_CONFIG[b.type].label})`
  ).join("\n");
}
      ...
      <div style={{ position: "relative", height: 56, display: "flex" }}>
        ...
        {blocks.map(b => {
          ...
          return (
            <div
              key={b.id}
              title={`${b.label}: ${fmtTime(b.startHour, b.startMin)} – ${fmtTime(b.endHour, b.endMin)}`}
              style={{
                position: "absolute",
                left: `${startPct}%`,
                width: `${widthPct}%`,
...
function BlockCard({ block, onRemove, onEdit }: { block: ScheduleBlock; onRemove: () => void; onEdit: (b: ScheduleBlock) => void }) {
  ...
  const save = () => {
    if (editLabel.trim()) onEdit({ ...block, label: editLabel.trim() });
    setEditing(false);
  };
  ...
  return (
    ...
    >
      ...
      {/* Type badge */}
      <span style={{
        fontSize: "0.62rem", fontWeight: 600, padding: "2px 7px", borderRadius: 20,
...
function AIPanel({ blocks }: { blocks: ScheduleBlock[] }) {
  ...
  const analyze = async () => {
    ...
    setAsked(true);
    try {
      const scheduleText = blocksToScheduleText(blocks);
      const prompt = `Here is my planned 24-hour daily schedule:\n\n${scheduleText}\n\nAnalyze this schedule for me. Look at: sleep quality, balance between work/school and rest, gym timing, free time placement, and overall structure. Give me direct, honest feedback and 2-3 specific improvements I can make. Keep it tight and actionable.`;
      const res = await api.post<{ message: string }>("/ai-assistant/chat", {
        userMessage: prompt,
        history: [],
      });
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: "0.76rem", fontWeight: 700, transition: "all 0.2s",
...
function AddBlockForm({ onAdd }: { onAdd: (b: ScheduleBlock) => void }) {
  ...
  const submit = () => {
    ...
    setError("");
    onAdd({ id: uid(), label: label.trim(), type, startHour: startH, startMin: startM, endHour: endH, endMin: endM });
    setLabel("");
  };
    ...
    <div style={{ background: "hsl(246 60% 10%)", borderRadius: 18, border: "1px solid rgba(255,255,255,0.07)", padding: "18px 20px" }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontSize: "0.72rem", fontWeight: selected ? 700 : 500,
                  transition: "all 0.15s",
      ...
      <button
        ...
        style={{
          ...
          background: "linear-gradient(135deg, #7467F0, #7c3aed)",
      ...
      >
        <Plus size={15} strokeWidth={2.5} /> Add to Schedule
      </button>
    </div>
  );
}
...
const TEMPLATES: { label: string; blocks: Omit<ScheduleBlock, "id">[] }[] = [
  {
    label: "🏫 School Day",
    blocks: [
...
const STORAGE_KEY = "flowbase-daily-schedule";
...
export default function DailySchedulePage() {
  ...
  const addBlock = (b: ScheduleBlock) => {
  ...
  const editBlock = (updated: ScheduleBlock) => {
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", margin: 0 }}>
            Plan your 24 hours — school, gym, sleep, free time and everything in between. Your AI coach will review your day.
          </p>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <p style={{ fontSize: "0.68rem", fontWeight: 600, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>24-Hour Overview <span style={{ fontWeight: 400 }}>(click a block to remove)</span></p>
            ...
            <div style={{ background: "hsl(246 60% 10%)", borderRadius: 18, border: "1px solid rgba(255,255,255,0.07)", padding: "16px 16px" }}>
              ...
              {sorted.length === 0 ? (
                <div style={{ textAlign: "center", padding: "28px 0", color: "rgba(255,255,255,0.2)" }}>
                  ...
                  <p style={{ margin: 0, fontSize: "0.8rem" }}>No blocks yet. Add your first time block on the left.</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {sorted.map(b => (
                    <BlockCard key={b.id} block={b} onRemove={() => removeBlock(b.id)} onEdit={editBlock} />
                  ))}
                </div>
              )}
            </div>
