...
function layoutNodes(nodes: DiagramNode[], diagramType: string): DiagramNode[] {
  ...
  if (diagramType === "mindmap" && nodes.length > 0) {
    ...
    const radius = 220;
    rest.forEach((n, i) => {
      n.x = center.x! + Math.cos(i * angleStep - Math.PI / 2) * radius;
      n.y = center.y! + Math.sin(i * angleStep - Math.PI / 2) * radius;
    });
  } else {
    nodes.forEach((n, i) => {
      const col = i % COLS;
      const row = Math.floor(i / COLS);
      n.x = START_X + col * (NODE_W + GAP_X);
      n.y = START_Y + row * (NODE_H + GAP_Y);
    });
  }
  return nodes;
}
...
function specToElements(spec: DiagramSpec): object[] {
  ...
  spec.nodes.forEach(node => {
    ...
    elements.push({
      ...
      versionNonce: rnd(),
      isDeleted: false,
      boundElements: [{ type: "text", id: textEid }],
      updated: Date.now(),
      link: null,
      locked: false,
    });
  ...
  spec.edges.forEach(edge => {
    ...
    const fx = (fromNode.x ?? 200) + 90;
    const fy = (fromNode.y ?? 200) + 70;
    const tx = (toNode.x ?? 200) + 90;
    const ty = toNode.y ?? 200;
    const arrowId = uid();
