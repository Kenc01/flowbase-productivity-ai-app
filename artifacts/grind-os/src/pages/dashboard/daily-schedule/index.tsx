...
interface CustomTemplate {
  id: string;
  label: string;
  blocks: Omit<ScheduleBlock, "id" | "date">[];
}
...
const BUILT_IN_TEMPLATES: { label: string; emoji: string; blocks: Omit<ScheduleBlock, "id" | "date">[] }[] = [
  {
...
function loadCompleted(date: string): Record<string, boolean> {
  try { return JSON.parse(localStorage.getItem(`grind_completed_${date}`) ?? "{}"); } catch { return {}; }
}
function saveCompleted(date: string, data: Record<string, boolean>) {
  localStorage.setItem(`grind_completed_${date}`, JSON.stringify(data));
}
...
function TimelineBar({ blocks, completed }: { blocks: ScheduleBlock[]; completed: Record<string, boolean> }) {
    ...
    <div style={{ position: "relative", width: "100%", background: "rgba(255,255,255,0.03)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
      <div style={{ position: "relative", height: 52 }}>
        {Array.from({ length: 24 }, (_, h) => h).map(h => (
          <div key={h} style={{
            position: "absolute", left: `${(h / 24) * 100}%`, top: 0, bottom: 0, width: "1px",
            background: h % 6 === 0 ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.04)",
          }} />
        ))}
        {blocks.map(b => {
          ...
          return (
            <div key={b.id} title={`${b.label}: ${fmtTime(b.startHour, b.startMin)} – ${fmtTime(b.endHour, b.endMin)}`}
              style={{
                ...
                top: "8px", bottom: "8px",
                background: done ? "rgba(16,185,129,0.7)" : cfg.color,
                borderRadius: 6, overflow: "hidden", cursor: "pointer",
                boxShadow: done ? "none" : `0 0 10px ${cfg.color}44`,
...
// ─── Block Row (Tracker View) ─────────────────────────────────────────────────

function BlockRow({
  block, completed, isTimerActive, elapsed,
  ...
  const progressPct = isTimerActive ? Math.min((elapsed / totalSecs) * 100, 100) : completed ? 100 : 0;
  ...
  return (
    ...
    >
      {/* Timer progress bar underlay */}
      {(isTimerActive || completed) && (
        <div style={{
          position: "absolute", left: 0, top: 0, bottom: 0,
          width: `${progressPct}%`,
          background: completed ? "rgba(16,185,129,0.1)" : `${cfg.color}18`,
      ...
      <button onClick={onToggleComplete} style={{
        ...
        background: completed ? "#10b981" : "rgba(255,255,255,0.07)",
        color: completed ? "#fff" : "rgba(255,255,255,0.3)",
        display: "flex", alignItems: "center", justifyContent: "center",
        transition: "all 0.18s", zIndex: 1,
      }}>
      ...
      <div style={{ flex: 1, minWidth: 0, zIndex: 1 }}>
        {editing ? (
          ...
          <span style={{
            fontSize: "0.86rem", fontWeight: 600, color: completed ? "rgba(255,255,255,0.45)" : "#f8fafc",
      ...
      {isTimerActive && (
        <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0, zIndex: 1 }}>
          ...
          <span style={{ fontSize: "0.85rem", fontWeight: 700, fontVariantNumeric: "tabular-nums", color: cfg.color }}>
            {fmtElapsed(elapsed)}
          </span>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 4, flexShrink: 0, zIndex: 1, opacity: hovered || isTimerActive ? 1 : 0, transition: "opacity 0.15s" }}>
        {editing ? (
          <>
            <button onClick={save} style={{ width: 26, height: 26, border: "none", borderRadius: 7, background: "rgba(16,185,129,0.2)", color: "#10b981", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Check size={11} />
            </button>
            <button onClick={() => setEditing(false)} style={{ width: 26, height: 26, border: "none", borderRadius: 7, background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <X size={11} />
            </button>
          </>
        ) : (
          <>
            {!completed && (
              isTimerActive ? (
                <button onClick={onStopTimer} title="Stop timer" style={{ width: 26, height: 26, border: "none", borderRadius: 7, background: "rgba(244,63,94,0.2)", color: "#F43F5E", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Square size={10} />
                </button>
              ) : (
                <button onClick={onStartTimer} title="Start focus timer" style={{ width: 26, height: 26, border: "none", borderRadius: 7, background: cfg.bg, color: cfg.color, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Play size={10} />
                </button>
              )
            )}
            <button onClick={() => setEditing(true)} style={{ width: 26, height: 26, border: "none", borderRadius: 7, background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.3)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Edit2 size={10} />
            </button>
            <button onClick={onRemove} style={{ width: 26, height: 26, border: "none", borderRadius: 7, background: "rgba(244,63,94,0.06)", color: "rgba(244,63,94,0.5)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Trash2 size={10} />
            </button>
          </>
        )}
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", background: "none", border: "none", cursor: "pointer", color: "#f8fafc" }}>
        ...
        <div style={{ padding: "0 16px 16px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ marginTop: 14, marginBottom: 12 }}>
            ...
            <input value={label} onChange={e => setLabel(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()}
              ...
              style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 9, padding: "8px 12px", color: "#f8fafc", fontSize: "0.82rem", outline: "none", fontFamily: "inherit" }} />
          </div>
              ...
              {(Object.keys(TYPE_CONFIG) as BlockType[]).map(t => {
                ...
                return (
                  <button key={t} onClick={() => setType(t)} style={{
                    display: "flex", alignItems: "center", gap: 4, padding: "4px 9px", borderRadius: 7, border: "none", cursor: "pointer",
                    ...
                    outline: sel ? `1.5px solid ${cfg.color}55` : "none",
                  }}>
                    {cfg.icon} {cfg.label}
                  </button>
                );
            ...
            {[{ lbl: "Start Time", h: startH, m: startM, setH: setStartH, setM: setStartM }, { lbl: "End Time", h: endH, m: endM, setH: setEndH, setM: setEndM }].map(({ lbl, h, m, setH, setM }) => (
              <div key={lbl}>
                ...
                <select value={`${h}:${m}`} onChange={e => { const [hh, mm] = e.target.value.split(":").map(Number); setH(hh); setM(mm); }}
                  style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 9, padding: "8px 10px", color: "#f8fafc", fontSize: "0.78rem", outline: "none", cursor: "pointer", fontFamily: "inherit" }}>
          ...
          <button onClick={submit} disabled={saving}
            style={{ width: "100%", padding: "9px", borderRadius: 10, border: "none", background: saving ? "rgba(116,103,240,0.4)" : "linear-gradient(135deg, #7467F0, #7c3aed)", color: "#fff", fontSize: "0.8rem", fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
    ...
    <div style={{ background: "rgba(245,158,11,0.06)", borderRadius: 14, border: "1px solid rgba(245,158,11,0.2)", padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <input value={prompt} onChange={e => setPrompt(e.target.value)}
          ...
          placeholder='e.g. "school day, wake at 6am, gym after school"'
          style={{ flex: 1, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 9, padding: "8px 12px", color: "#f8fafc", fontSize: "0.8rem", outline: "none", fontFamily: "inherit" }}
        />
        <button onClick={send} disabled={loading || !prompt.trim()}
          style={{ width: 36, height: 36, borderRadius: 9, border: "none", background: loading || !prompt.trim() ? "rgba(245,158,11,0.2)" : "linear-gradient(135deg, #f59e0b, #f97316)", color: "#fff", cursor: loading || !prompt.trim() ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          {loading ? <Loader2 size={14} style={{ animation: "spin 0.8s linear infinite" }} /> : <Send size={13} />}
        </button>
      </div>
      {reply && (
        <div style={{ marginTop: 10, padding: "10px 12px", background: "rgba(255,255,255,0.04)", borderRadius: 9, fontSize: "0.78rem", color: "rgba(255,255,255,0.7)", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
          {reply}
        </div>
      )}
    </div>
...
function TemplatesPanel({
  blocks, onApply, onSaveCustom, customTemplates, onDeleteCustom, saving,
}: {
  blocks: ScheduleBlock[];
  onApply: (tpl: { label: string; blocks: Omit<ScheduleBlock, "id"|"date">[] }) => void;
  ...
  const doSave = () => {
    if (saveName.trim() && blocks.length > 0) {
    ...
    <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", padding: "14px 16px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        {blocks.length > 0 && (
          <button onClick={() => setShowSave(s => !s)}
            style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 8, border: "1px solid rgba(116,103,240,0.3)", background: "rgba(116,103,240,0.1)", color: "#7467F0", fontSize: "0.72rem", fontWeight: 600, cursor: "pointer" }}>
      ...
      {showSave && (
        <div style={{ display: "flex", gap: 7, marginBottom: 12 }}>
          <input autoFocus value={saveName} onChange={e => setSaveName(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter") doSave(); if (e.key === "Escape") setShowSave(false); }}
            placeholder='Template name, e.g. "My School Day"'
            style={{ flex: 1, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(116,103,240,0.4)", borderRadius: 8, padding: "7px 11px", color: "#f8fafc", fontSize: "0.78rem", outline: "none", fontFamily: "inherit" }}
          />
          <button onClick={doSave} disabled={!saveName.trim()}
            style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: saveName.trim() ? "#7467F0" : "rgba(116,103,240,0.3)", color: "#fff", fontSize: "0.78rem", fontWeight: 700, cursor: saveName.trim() ? "pointer" : "not-allowed" }}>
            <Save size={12} />
          </button>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {BUILT_IN_TEMPLATES.map(tpl => (
          <button key={tpl.label} onClick={() => !saving && onApply(tpl)} disabled={saving}
            style={{
              display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", borderRadius: 10,
              border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.04)",
              color: "#f8fafc", fontSize: "0.78rem", fontWeight: 600, cursor: saving ? "not-allowed" : "pointer",
          ...
          <>
            ...
            {customTemplates.map(tpl => (
              <div key={tpl.id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <button onClick={() => !saving && onApply(tpl)} disabled={saving}
                  style={{
                    flex: 1, display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", borderRadius: 10,
                    border: "1px solid rgba(116,103,240,0.2)", background: "rgba(116,103,240,0.07)",
                    color: "#f8fafc", fontSize: "0.78rem", fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", textAlign: "left", transition: "all 0.15s",
                  }}
                ...
                <button onClick={() => onDeleteCustom(tpl.id)}
                  style={{ width: 28, height: 28, borderRadius: 8, border: "none", background: "rgba(244,63,94,0.08)", color: "rgba(244,63,94,0.5)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Trash2 size={11} />
                </button>
              </div>
            ))}
...
function ProgressHeader({ blocks, completed }: { blocks: ScheduleBlock[]; completed: Record<string, boolean> }) {
  const total = blocks.length;
  const done = blocks.filter(b => completed[b.id]).length;
    ...
    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
      ...
      {currentBlock && (
        <div style={{ flex: 1, minWidth: 160, background: TYPE_CONFIG[currentBlock.type].bg, borderRadius: 12, border: `1px solid ${TYPE_CONFIG[currentBlock.type].color}44`, padding: "12px 16px" }}>
          ...
          <div style={{ fontSize: "0.68rem", color: "rgba(255,255,255,0.35)", marginTop: 3 }}>
            Until {fmtTime(currentBlock.endHour, currentBlock.endMin)}
          </div>
        </div>
      )}
...
export default function DailySchedulePage() {
  ...
  const [completed, setCompleted] = useState<Record<string, boolean>>(() => loadCompleted(date));
  ...
  useEffect(() => {
    const iv = setInterval(loadBlocks, 8000);
  ...
  const removeBlock = async (id: string) => {
    ...
    const newC = { ...completed }; delete newC[id];
    setCompleted(newC); saveCompleted(date, newC);
  ...
  const clearAll = async () => {
    setBlocks([]); setCompleted({}); saveCompleted(date, {});
    setActiveTimerId(null); setElapsed(0);
    try { await api.delete(`/daily-schedule?date=${date}`); } catch { loadBlocks(); }
  };

  const applyTemplate = async (tpl: { label: string; blocks: Omit<ScheduleBlock, "id" | "date">[] }) => {
    setSaving(true);
    try {
      await api.delete(`/daily-schedule?date=${date}`);
      setCompleted({}); saveCompleted(date, {});
      setActiveTimerId(null); setElapsed(0);
  ...
  const toggleComplete = (id: string) => {
    const newC = { ...completed, [id]: !completed[id] };
    if (newC[id] && activeTimerId === id) { setActiveTimerId(null); setElapsed(0); }
    setCompleted(newC); saveCompleted(date, newC);
  };
  ...
  const doneCount = blocks.filter(b => completed[b.id]).length;

  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "28px 24px", background: "var(--fb-bg)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            ...
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: "0.82rem", margin: 0 }}>
              Plan your day, start your blocks, and track every session.
            </p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {blocks.length > 0 && (
              <button onClick={clearAll}
                style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 13px", borderRadius: 9, border: "1px solid rgba(244,63,94,0.2)", background: "rgba(244,63,94,0.06)", color: "#F43F5E", fontSize: "0.74rem", fontWeight: 600, cursor: "pointer" }}>
                <Trash2 size={12} /> Clear Day
              </button>
            )}
            <button onClick={loadBlocks}
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 13px", borderRadius: 9, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.5)", fontSize: "0.74rem", cursor: "pointer" }}>
        ...
        {/* Progress stats */}
        {blocks.length > 0 && <ProgressHeader blocks={blocks} completed={completed} />}

        {/* Timeline */}
        <div style={{ marginBottom: 20 }}>
          <TimelineBar blocks={blocks} completed={completed} />
        </div>
          ...
          return (
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", borderRadius: 14, background: `${cfg.color}18`, border: `1px solid ${cfg.color}44`, marginBottom: 20 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <span style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.4)", marginLeft: 10 }}>
              ...
              <button onClick={() => { toggleComplete(activeTimerId); stopTimer(); }}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 9, border: "none", background: "#10b981", color: "#fff", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}>
                <CheckCircle2 size={13} /> Done
              </button>
              <button onClick={stopTimer}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", borderRadius: 9, border: "1px solid rgba(244,63,94,0.3)", background: "rgba(244,63,94,0.1)", color: "#F43F5E", fontSize: "0.78rem", fontWeight: 600, cursor: "pointer" }}>
                <Square size={11} /> Stop
              </button>
            </div>
          );
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 4 }}>
                  {["upcoming", "active", "done"].map(s => {
                    const count = sorted.filter(b => {
                      if (completed[b.id]) return s === "done";
