...
function renderMarkdown(text: string): React.ReactNode[] {
  ...
  while (i < lines.length) {
    const line = lines[i];
    if (line.startsWith("### ")) {
      nodes.push(<div key={i} style={{ fontSize: 12, fontWeight: 700, color: "#a78bfa", textTransform: "uppercase", letterSpacing: "0.07em", marginTop: 10, marginBottom: 3 }}>{inlineRender(line.slice(4))}</div>);
    } else if (line.startsWith("## ")) {
      nodes.push(<div key={i} style={{ fontSize: 14, fontWeight: 700, color: "var(--fb-text)", marginTop: 12, marginBottom: 4, fontFamily: "Outfit, sans-serif" }}>{inlineRender(line.slice(3))}</div>);
    } else if (line.startsWith("# ")) {
      nodes.push(<div key={i} style={{ fontSize: 15, fontWeight: 800, color: "var(--fb-text)", marginTop: 12, marginBottom: 6, fontFamily: "Outfit, sans-serif" }}>{inlineRender(line.slice(2))}</div>);
    } else if (line.match(/^[-*] /)) {
      const items: string[] = [];
      while (i < lines.length && lines[i].match(/^[-*] /)) { items.push(lines[i].slice(2)); i++; }
      nodes.push(
        <ul key={`ul-${i}`} style={{ margin: "5px 0", paddingLeft: 0, listStyle: "none" }}>
          {items.map((it, j) => (
            <li key={j} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 4 }}>
              <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#7467F0", flexShrink: 0, marginTop: 8 }} />
              <span style={{ color: "var(--fb-text)" }}>{inlineRender(it)}</span>
            </li>
          ))}
        </ul>
      );
      ...
      nodes.push(
        <ol key={`ol-${i}`} style={{ margin: "5px 0", paddingLeft: 20 }}>
          {items.map((it, j) => (
            <li key={j} style={{ marginBottom: 4, color: "var(--fb-text)" }}>{inlineRender(it)}</li>
          ))}
        </ol>
      );
      continue;
      ...
      nodes.push(<div key={i} style={{ lineHeight: 1.65, color: "var(--fb-text)" }}>{inlineRender(line)}</div>);
    }
    i++;
  }
  return nodes;
...
function inlineRender(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**"))
      return <strong key={i} style={{ fontWeight: 700, color: "var(--fb-text)" }}>{part.slice(2, -2)}</strong>;
    if (part.startsWith("*") && part.endsWith("*") && part.length > 2)
      return <em key={i} style={{ color: "var(--fb-text-muted)" }}>{part.slice(1, -1)}</em>;
    if (part.startsWith("`") && part.endsWith("`"))
      return <code key={i} style={{ background: "rgba(116,103,240,.2)", color: "#a78bfa", borderRadius: 4, padding: "1px 5px", fontSize: "0.88em", fontFamily: "monospace" }}>{part.slice(1, -1)}</code>;
    return <span key={i} style={{ color: "var(--fb-text)" }}>{part}</span>;
  });
}
...
function ToolResultCard({ action, onNavigate }: { action: Action; onNavigate: (h: string) => void }) {
  ...
  const bgColor = isCreate
    ? (action.success ? "rgba(16,185,129,.1)" : "rgba(244,63,94,.1)")
    : "var(--fb-surface-hover)";
  const borderColor = isCreate
    ? (action.success ? "rgba(16,185,129,.28)" : "rgba(244,63,94,.28)")
    : "var(--fb-border)";
  const dividerColor = isCreate
    ? (action.success ? "rgba(16,185,129,.18)" : "rgba(244,63,94,.18)")
    : "var(--fb-border)";

  return (
    <div style={{ marginTop: 8, background: bgColor, border: `1px solid ${borderColor}`, borderRadius: 12, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", borderBottom: `1px solid ${dividerColor}` }}>
        <div style={{ width: 20, height: 20, borderRadius: 5, background: `${meta.color}22`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          {isCreate
            ? <CheckCircle2 size={11} color={action.success ? "#10B981" : "#F43F5E"} />
            : <Icon size={11} color={meta.color} />}
        </div>
        <span style={{ fontSize: 10, fontWeight: 700, color: isCreate ? (action.success ? "#10B981" : "#F43F5E") : meta.color, letterSpacing: "0.06em", textTransform: "uppercase" }}>
          {isCreate ? (action.success ? "Created" : "Failed") : `Read ${meta.label}`}
        </span>
        {action.link && (
          <button onClick={() => onNavigate(action.link!)}
            style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 3, background: "none", border: "none", cursor: "pointer", fontSize: 11, color: "#7467F0", fontWeight: 600, padding: "2px 6px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {result.slice(0, 6).map((task: any, i: number) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 5, height: 5, borderRadius: "50%", background: PRIORITY_COLOR[task.priority] ?? "var(--fb-text-muted)", flexShrink: 0 }} />
                <span style={{ fontSize: 12, color: "var(--fb-text)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{task.title}</span>
                <span style={{ fontSize: 10, color: "var(--fb-text-muted)", flexShrink: 0 }}>{task.columnName}</span>
              </div>
            ))}
        ...
        {action.tool === "get_schedule" && Array.isArray(result) && result.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {result.slice(0, 5).map((ev: any, i: number) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 600, color: "#06B6D4", minWidth: 60 }}>{ev.date}</span>
                <span style={{ fontSize: 12, color: "var(--fb-text)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.title}</span>
                <span style={{ fontSize: 10, background: "rgba(6,182,212,.15)", color: "#06B6D4", borderRadius: 4, padding: "1px 5px" }}>{ev.type}</span>
              </div>
            ))}
        ...
        {action.tool === "get_daily_schedule" && result?.blocks && result.blocks.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {result.blocks.slice(0, 6).map((b: any, i: number) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 10, color: "#8B5CF6", fontWeight: 600, minWidth: 70 }}>{fmtTime(b.startHour, b.startMin)}</span>
                <span style={{ fontSize: 12, color: "var(--fb-text)" }}>{b.label}</span>
                <span style={{ fontSize: 10, color: "var(--fb-text-muted)", marginLeft: "auto" }}>{b.type}</span>
              </div>
            ))}
          </div>
        )}
...
function TypingIndicator({ activeTool }: { activeTool?: string | null }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 16px" }}>
      {meta ? (
        <>
          <Loader2 size={12} color={meta.color} style={{ animation: "j-spin 1s linear infinite", flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: "var(--fb-text-muted)" }}>Reading {meta.label.toLowerCase()}…</span>
        </>
      ) : (
        [0, 1, 2].map((i) => (
          <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#7467F0", opacity: 0.7, animation: `j-bounce 1.2s ease-in-out ${i * 0.18}s infinite` }} />
        ))
      )}
    </div>
  );
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="j-copy-btn" title="Copy"
      style={{ width: 24, height: 24, borderRadius: 6, background: "var(--fb-muted)", border: "1px solid var(--fb-border)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .15s", opacity: 0, position: "absolute", top: 8, right: 8 }}>
      {copied ? <Check size={11} color="#10B981" /> : <Copy size={11} color="var(--fb-text-muted)" />}
    </button>
  );
}
...
function VoiceModeOverlay({ status, lastText, lastRole, masterName, onStop }: {
  ...
  const cfg: Record<VoiceAgentStatus, { label: string; color: string; pulse: boolean }> = {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 50, background: "rgba(6, 5, 18, 0.98)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      <div style={{ marginBottom: 28, textAlign: "center" }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.22em", color: "rgba(255,255,255,.2)", textTransform: "uppercase", marginBottom: 5 }}>JARVIS VOICE</div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,.35)" }}>Master {name}</div>
      </div>

      <div style={{ position: "relative", marginBottom: 32 }}>
        {c.pulse && [1, 2, 3].map((i) => (
          <div key={i} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 90 + i * 48, height: 90 + i * 48, borderRadius: "50%", border: `1px solid ${c.color}`, opacity: 0.12 / i, animation: `vc-ring 2s ease-out ${i * 0.35}s infinite` }} />
        ))}
        <div style={{ width: 100, height: 100, borderRadius: "50%", background: `radial-gradient(circle at 35% 35%, ${c.color}cc, ${c.color}33)`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 40px ${c.color}44`, animation: c.pulse ? "vc-breathe 1.8s ease-in-out infinite" : "none" }}>
          {status === "listening"  && <Mic size={38} color="#fff" strokeWidth={1.6} />}
          {status === "thinking"   && <Brain size={38} color="#fff" strokeWidth={1.6} style={{ animation: "j-spin 2s linear infinite" }} />}
        ...
        {(status === "listening" || status === "speaking") && (
          <div style={{ display: "flex", gap: 4, alignItems: "center", justifyContent: "center", marginTop: 18 }}>
            {[0,1,2,3,4,5,6].map((i) => (
              <div key={i} style={{ width: 3, borderRadius: 99, background: c.color, animation: `vc-wave 1s ease-in-out ${i * 0.1}s infinite`, height: `${10 + Math.sin(i * 1.2) * 8 + 6}px` }} />
            ))}
          </div>
        )}
      </div>

      <p style={{ fontSize: 22, fontWeight: 700, color: "#fff", margin: 0, marginBottom: 8, fontFamily: "Outfit, sans-serif" }}>{c.label}</p>

      <div style={{ minHeight: 52, maxWidth: 480, width: "90%", textAlign: "center", marginBottom: 40 }}>
        {lastText ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 5, alignItems: "center" }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", color: "rgba(255,255,255,.25)", textTransform: "uppercase" }}>
              {lastRole === "agent" ? "JARVIS" : "You said"}
            </span>
            <p style={{ fontSize: 14, color: lastRole === "agent" ? "#06B6D4" : "rgba(255,255,255,.6)", margin: 0, fontStyle: "italic", lineHeight: 1.6 }}>"{lastText}"</p>
          </div>
        ) : (
      ...
      <button onClick={onStop}
        style={{ display: "flex", alignItems: "center", gap: 10, padding: "13px 30px", borderRadius: 99, background: "#F43F5E", border: "none", cursor: "pointer", color: "#fff", fontSize: 14, fontWeight: 700, boxShadow: "0 4px 20px rgba(244,63,94,.45)", transition: "all .15s" }}
        onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1.04)"; }}
        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1)"; }}>
        <PhoneOff size={16} /> End Voice Session
      </button>
      <p style={{ fontSize: 10, color: "rgba(255,255,255,.15)", marginTop: 14, letterSpacing: "0.08em" }}>SPEAK NATURALLY — JARVIS RESPONDS AUTOMATICALLY</p>
...
export default function AIAssistantPage() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    if (lc.includes("task") || lc.includes("todo")) setActiveTool("get_tasks");
    else if (lc.includes("schedul") || lc.includes("calendar") || lc.includes("today") || lc.includes("event")) setActiveTool("get_schedule");
  ...
  const startRecording = async () => {
    ...
    } catch { setVoiceError("Microphone access denied. Please allow microphone use in your browser."); }
  };
  ...
  const handleVoiceMessage = useCallback((msg: VoiceAgentMessage) => {
    setLastVoiceText(msg.text); setLastVoiceRole(msg.role);
  ...
  const { connect, disconnect } = useVoiceAgent({
    ...
    onMessage: handleVoiceMessage, onStatusChange: handleVoiceStatusChange, onError: handleVoiceError,
  });

  const handleStartVoiceMode = useCallback(() => { setVoiceError(null); setLastVoiceText(""); setVoiceMode(true); connect(); }, [connect]);
  const handleStopVoiceMode = useCallback(() => { disconnect(); setVoiceMode(false); setLastVoiceText(""); }, [disconnect]);
